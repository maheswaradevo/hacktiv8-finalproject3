...
var (
	...
	VIEW_CATEGORIES      = "SELECT c.id, c.type, c.updated_at, c.created_at, t.id, t.title, t.description, t.user_id, t.category_id, t.created_at, t.updated_at FROM `tasks` t JOIN  `categories` c ON c.id = t.category_id ORDER BY c.created_at DESC;"
...
func (ctg CategoriesImplRepo) ViewCategories(ctx context.Context) (model.PeopleCategoriesJoined, error) {
	...
	var peopleCategories model.PeopleCategoriesJoined
	for rows.Next() {
		personCategories := model.CategoriesUserJoined{}
		err := rows.Scan(
			...
			&personCategories.Task.TaskID,
			&personCategories.Task.Title,
			&personCategories.Task.Description,
			&personCategories.Task.UserID,
			&personCategories.Task.CategoryID,
			&personCategories.Task.CreatedAt,
			&personCategories.Task.UpdatedAt,
		)
		if err != nil {
