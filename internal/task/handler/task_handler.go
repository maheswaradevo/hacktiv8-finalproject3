...
func NewTaskHandler(r *gin.RouterGroup, ts task.TaskService) *gin.RouterGroup {
	...
	taskProtectedRoute := delivery.r.Group("/tasks", middleware.AuthMiddleware())
	{
		...
		taskProtectedRoute.Handle(http.MethodPut, "/:taskId", delivery.updateTask)
...
func (cmth *TaskHandler) updateTask(c *gin.Context) {
	data := dto.EditTaskRequest{}
	...
	res, err := cmth.ts.UpdateTask(c, TaskIDConv, userID, &data)
	if err != nil {
...
func (cmth *TaskHandler) updateTaskCategory(c *gin.Context) {
	data := dto.EditTaskCategoryRequest{}
	...
	res, err := cmth.ts.UpdateTaskCategory(c, TaskIDConv, userID, &data)
	if err != nil {
		log.Printf("[updateTask] failed to update task, id: %v, err: %v", TaskIDConv, err)
		errResponse := utils.NewErrorResponse(c.Writer, err)
		c.JSON(errResponse.Code, errResponse)
